...
class StandardTestCase(unittest.TestCase):
    ...
    def standard_test(self, options={}):
        ...
        if options:
            command += StandardTestCase._dict_options_to_list(options)
        subprocess.call(command)

        expected = 'N files: ' + str(len(self.input_files('xlsforms')))
        actual = 'N files: ' + str(len(self.output_files()))
        self.assertEqual(expected, actual)


class StandardTestCase1(StandardTestCase):
